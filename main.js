/**
 * Context Vault — Main Electron Process
 * 
...
async function approveFileAccess(filePath) {
  ...
  const result = await dialog.showMessageBox({
    type: 'warning',
    title: 'Context Vault Vault — File Access Request',
...
function connectToMCPClient(configPath, clientName) {
  if (docsPaths.length === 0) {
    dialog.showErrorBox('Context Vault Vault', 'Please select a folder first.');
    return;
  }
  ...
  try {
    ...
    // Add Context Vault entry
    config.mcpServers['context-vault-vault'] = {
      command: 'npx',
    ...
    dialog.showMessageBox({
      type: 'info',
      title: 'Connected!',
      message: `Context Vault Vault connected to ${clientName}`,
...
async function startVault() {
  try {
    ...
    await stopVault();
    dialog.showErrorBox('Context Vault', `Failed to start: ${err.message}`);
  }
}
